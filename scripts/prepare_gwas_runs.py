...
def parse_arguments():
    description = "Script to prepare power calculation GWAS runs across parameters for VCF. \nThis script reads a " \
                  ...
                  "individual GWAS scripts used to run them all sequentially or to submitted them as jobs" \
...
def _main():
    ...
    if args.pyseer_model != "LMM":
        if args.pyseer_model != "ENET":
            logging.error(f'--pyseer_model must be either \"LMM\" or \"ENET\"')
            sys.exit(-1)
    ...
    scripts = [code_directory + 'sample_casual_variants_from_vcf.py',
               code_directory + 'simulate_phenotype_using_gcta.py',
               code_directory + 'simulate_binary_phenotype_vcf.py',
    ...
    input_files = [args.input_vcf, args.parameters_file, args.pastml_steps_file]
    ...
    for input_file in input_files:
        if not os.path.exists(input_file):
            logging.error(f'Input file {input_file} not found. Run vcf_to_plink_files.py first.')
            sys.exit(-1)
    ...
    if len(allele_frequency_from) != len(allele_frequency_to):
        logging.error(f'Make sure allele_frequency_from and allele_frequency_to have the same length in '
                      f'{args.parameters_file}.')
        sys.exit(-1)
    ...
    if len(homoplasy_steps_from) != len(homoplasy_steps_to):
        logging.error(f'Make sure homoplasy_steps_from and homoplasy_steps_to have the same length in '
                      f'{args.parameters_file}.')
        sys.exit(-1)
                    ...
                    for ncv in number_causal_variants:
                        for sr in list(range(1, int(sampling_repetitions)+1, 1)):
                            for ss in sample_size:
                                ...
                                causal_regions = output_dir + combination_id + '.causal_regions.txt'
                                ...
                                command_line += 'then\n'
                                command_line += code_directory + 'sample_casual_variants_from_vcf.py' \
                                                + ' --input_vcf ' + args.input_vcf \
                                                + ' --number_variants ' + str(ncv) \
                                                + ' --allele_frequency_range ' + str(af) \
                                                + ' --degree_homoplasy_range ' + str(hs) \
                                                + ' --effect_size ' + str(es) \
                                                + ' --output_gcta_file ' + causal_variants \
                                                + ' --pastml_steps_file ' + args.pastml_steps_file
                                if args.burden is not None:
                                    command_line += ' --regions_file ' + args.burden \
                                                    + ' --output_regions_file ' + causal_regions
                                command_line += '\n'
                                command_line += 'fi\n'
                                command_line += 'if [ -f ' + causal_variants + ' ]\n'
                                command_line += 'then\n'
                                ...
                                if args.phenotype_type == "binary":
                                    ...
                                    if len(heritability) > 1:
                                        ...
                                        command_line += code_directory + 'simulate_binary_phenotype_vcf.py' \
                                                        + ' --input_vcf ' + args.input_vcf \
                                ...
                                if args.phenotype_type == "quantitative":
                                    command_line += code_directory + 'subsample_vcf_samples.py' \
                                                    + ' --input_vcf ' + args.input_vcf \
                                                    + ' --causal-loci ' + causal_variants \
                                                    + ' --subset_size ' + str(ss) \
                                ...
                                for ir in list(range(1, int(simulation_repetitions)+1, 1)):
                                    ...
                                    command_line += ' --vcf ' + args.input_vcf \
