...
plot = plot + scale_color_manual(values=c("#F0F0F0", "#C0C0C0", "#909090", "#505050", "#080808"))
plot = plot + geom_line(aes(color=allele_frequency))
plot = plot + geom_point(aes(color=allele_frequency, shape=effect_size))
plot = plot + labs(title="Sub-sampling GWAS power calculations", subtitle = "For allele frequency/effect size combinations across sample sizes", x = "Sample size", y = "-log10(p-value)")
