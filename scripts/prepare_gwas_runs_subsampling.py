...
def parse_arguments():
    ...
    group.add_argument(
        ...
        help="Pyseer model to run. To choose between: \"LMM\" (Linear Mixed Models) or \"ENET\" (Elastic Net). "
             "Default: LMM",
        required=False, default='LMM', metavar="CODE_DIR"
    )
    # NOTE: quantitative phenotypes not supported for sub-sampling approach
    # group.add_argument(
    #     "-w", "--phenotype_type", action="store", dest="phenotype_type",
    #     help="Type of phenotype in --phenotype. To choose between: \"binary\" or \"quantitative\". [Default: binary]",
    #     required=False, default='binary', metavar="CODE_DIR"
    # )
    group.add_argument(
...
def _main():
    ...
    input_files = [args.input_vcf, args.parameters_file, args.causal_loci, args.phenotype]
    ...
    phenotype_samples = []
    next(file)  # skipping first header line
    for line in file:
    ...
    if len(matched_samples) != len(vcf_samples):
        ...
        print("vcf_samples " + '\n'.join(vcf_samples))
        print("phenotype_samples " + '\n'.join(phenotype_samples))
        print("len(vcf_samples) " + str(len(vcf_samples)))
        print("len(phenotype_samples) " + str(len(phenotype_samples)))
        sys.exit(-1)
