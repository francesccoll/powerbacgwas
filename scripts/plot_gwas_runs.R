...
if(opt$plot_type == 1){
  ...
  plot = plot + labs(title="GWAS power calculations", subtitle = "For allele frequency/effect size combinations across sample sizes", x = "Sample size", y = y_label)
...
if(opt$plot_type == 2){
  ...
  plot = plot + labs(title="GWAS power calculations", subtitle = "For allele frequency/heritability combinations across sample sizes", x = "Sample size", y = y_label)
  plot = plot + scale_x_continuous(breaks=sample_sizes, labels=as.character(sample_sizes))
  if(opt$plot_metrics == 2){ plot = plot + geom_hline(yintercept=genome_wide_sig_level, linetype="dashed", size = axis_lines_width); }
  if(opt$plot_metrics == 1){ plot = plot + geom_hline(yintercept=80, linetype="dashed", size = axis_lines_width); }
  if(!is.na(opt$ylim_to)){
...
if(opt$plot_type == 3){
  ...
  plot = plot + labs(title="GWAS power calculations", subtitle = paste("For homoplasy level/effect size combinations across sample sizes (allele frequency: ",opt$allele_frequency,")",sep=""), x = "Sample size", y = y_label)
  plot = plot + scale_x_continuous(breaks=sample_sizes, labels=as.character(sample_sizes))
  if(opt$plot_metrics == 2){ plot = plot + geom_hline(yintercept=genome_wide_sig_level, linetype="dashed", size = axis_lines_width); }
  if(opt$plot_metrics == 1){ plot = plot + geom_hline(yintercept=80, linetype="dashed", size = axis_lines_width); }
  plot = plot + theme(axis.text.x = element_text(size = 7))             # x-axis text size
