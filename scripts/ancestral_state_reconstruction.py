...
def parse_arguments():
    description = "Script to perform ancestral state reconstruction using PastML"
    parser = argparse.ArgumentParser(description=description)

    group = parser.add_argument_group('Required arguments')
    group.add_argument(
        "-v", "--input_vcf_table", action="store", dest="input_vcf_table",
        help="matrix of variants obtained from a multi-sample VCF file using the script vcf_to_matrix.py",
    ...
    group.add_argument(
        ...
        help="output table with variants and their ancestral nodes",
        required=True, metavar="OUTPUT_TABLE"
    )
    group.add_argument(
        "-s", "--output_steps", action="store", dest="output_steps",
        help="output table with number of homoplasies per variant",
    ...
    group.add_argument(
        "-b", "--split_by_size", action="store", dest="split_by_size",
        help="Size of slices input matrix of variants is split by",
        required=False, metavar="SPLIT_BY", default=1000
    )
    group.add_argument(
    ...
    group.add_argument(
        "-d", "--pastml_dir", action="store", dest="pastml_dir",
        help="Temporary directory to save PastML intermediate files.", required=False, default='tmp_pastml_dir',
        metavar="PASTML_DIR"
    )
    group.add_argument(
        "-r", "--rm_tmp", action="store_true", dest="rm_tmp",
        help="Whether to remove PastML temporary directory and intermediate files. Default: False", required=False,
        default=False
    )
...
def _main():
    ...
    input_files = [args.input_vcf_table, args.input_tree]
    for input_file in input_files:
        if not os.path.exists(input_file):
            logging.error(f'Input file {input_file} not found!')
            sys.exit(-1)

    # Slicing vcf csv table: to run each slice separately using PastML
    logging.info(f"Reading {args.input_vcf_table}. This may take several minutes...")
    df = pd.read_csv(args.input_vcf_table, sep='\t')
    logging.info(f"Reading {args.input_vcf_table}. DONE")
    print("Number of samples: " + str(df.shape[0]) + " number of variants " + str(df.shape[1]))

    logging.info(f"Slicing {args.input_vcf_table}. This may take several minutes...")
    # by = 1000
    by = int(args.split_by_size)
    number_columns = df.shape[1]
    column_names = df.columns.values.tolist()
    input_vcf_table_slices = []
    for c in range(1, number_columns, by):
        start = c
        end = c + by - 1
        end = df.shape[1] - 1 if end > number_columns else end
        slice_name = '_slice_' + str(start) + '_' + str(end)
        slice_file_name = args.input_vcf_table + slice_name
        input_vcf_table_slices.append(slice_file_name)
    ...
    logging.info(f"Slicing {args.input_vcf_table}. DONE")

    # Running PastML (Ancestral reconstruction) for each slice - multi-processing
    pastml_dir = args.pastml_dir
    ...
    command_lines = []
    for input_vcf_table_slice in input_vcf_table_slices:
        command_line = ["pastml", "--tree", args.input_tree,
                        "--data", input_vcf_table_slice,
                        "--out_data", input_vcf_table_slice + '_pastml',
                        "--work_dir", pastml_dir,
                        "--prediction_method", "ACCTRAN"]
        command_line_string = ' '.join(command_line)
        if not os.path.isfile(input_vcf_table_slice + '_pastml'):  # if PastML output does not exist
    ...
    for input_vcf_table_slice in input_vcf_table_slices:
        if not os.path.isfile(input_vcf_table_slice + '_pastml_processed'):
            df_pastml_slice = pd.read_csv(input_vcf_table_slice + '_pastml', sep='\t', dtype=object)
            # extracting VCF table columns names
            df_vcf_slice = pd.read_csv(input_vcf_table_slice, sep='\t', dtype=object)
            vcf_column_names_prv = df_vcf_slice.columns.values.tolist()
            vcf_column_names_new = list()
            for cn in vcf_column_names_prv:
                # vcf_column_names_new.append(cn.replace('.', ''))
                vcf_column_names_new.append(cn.replace('.', '').replace(',', '').replace('@', ''))
            vcf_column_names_new[0] = 'node'
            # matching order of columns between VCF table and PASTML output
            df_pastml_slice = df_pastml_slice.reindex(columns=vcf_column_names_new)
            ...
            df_pastml_slice.to_csv(input_vcf_table_slice + '_pastml_processed', header=True, sep='\t', index=False)
    ...
    if not os.path.isfile(args.output_table + '_tmp'):
        ...
        for input_vcf_table_slice in input_vcf_table_slices:
            paste_command += input_vcf_table_slice + '_pastml_processed '
    ...
    if not os.path.isfile(args.output_steps):
        ...
        for variant_id in variant_ids:
            params_file = pastml_dir + '/params.character_' + variant_id.replace('.', '') + '.method_ACCTRAN.tab'
    ...
    if args.rm_tmp:
        ...
        files_to_rm = list()
        for input_vcf_table_slice in input_vcf_table_slices:
            files_to_rm.append(input_vcf_table_slice)
            files_to_rm.append(input_vcf_table_slice + '_pastml')
            files_to_rm.append(input_vcf_table_slice + '_pastml_processed')
        files_to_rm.append(args.output_table + '_tmp')
        files_to_rm.append(pastml_dir)
        for file_to_rm in files_to_rm:
            run_command_shell_string(' '.join(["rm -r -f", file_to_rm]))
        logging.info(f"Deleting temporary files. DONE.")
