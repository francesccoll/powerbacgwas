...
def printHelp() {
  ...
  Additional options:
    ...
    Outputs:
      --outprefix                   Output prefix used to name intermediate and final output files.
    ...
    Pipeline Options:
      ...
      --run_vcf_ast                 Run ancestral state reconstruction for VCF. --run_prepare_vcf must be run first. 
				  Must also specify the --outprefix used in --run_prepare_vcf.
      --run_vcf_regions_ast         Calculate number of independent variant acquisitions (homoplasies) at the region level (e.g. gene). Needed for burden testing power calculations.
                                    Must also specify the --outprefix used in --run_prepare_vcf, and --regions inputs.
      --run_pangenome_ast           Run ancestral state reconstruction for pan-genome. --run_prepare_pangenome must be run first. Must also specify
                                    the --outprefix used in --run_prepare_pangenome. (Default: false)
      --run_subsampling_vcf         Run the sub-sampling approach for individual VCF variants. Must also specify --outprefix, --pangenome and --tree inputs.
      --run_subsampling_pangenome   Run the sub-sampling approach for the pan-genome. Must also specify --outprefix, --vcf and --tree inputs.
      --run_subsampling_burden	  Run the sub-sampling approach for burden testing. Must also specify --outprefix, --vcf, --tree and --regions inputs.
      --run_phensim_vcf             Run the phenotype simulation approach for individual VCF variants. Must also specify --outprefix, --pangenome and --tree inputs.
      --run_phensim_pangenome       Run the phenotype simulation approach for the pan-genome. Must also specify --outprefix, --vcf and --tree inputs.
      --run_phensim_burden	  Run the phenotype simulation approach for burden testing. Must also specify --outprefix, --vcf, --tree and --regions inputs.
